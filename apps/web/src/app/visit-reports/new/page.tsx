...
function NewVisitReportPageContent() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!visitPlanId || !visitPlanItemId || !partyId) {
        throw new Error("Visit Plan, plan item, and party IDs are required");
      }

      await api.post<ApiResponse<{ _id: string }>>("/visit-reports", {
        visitPlanId,
        visitPlanItemId,
        partyId,
        camId: user.id,
      ...
      setError(getApiErrorMessage(err, "Unable to save Visit Report"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Record the visit outcome, customer requirement, product fit, and next follow-up."
        actions={
          <Link href={`/visit-plans/${visitPlanId}`} className="btn-secondary">
            Back to Plan
          </Link>
        }
      />
      ...
      <form onSubmit={submit} className="app-card space-y-6 p-6">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <InputField
            ...
            onChange={(event) => update("visitedAt", event.target.value)}
          />
          ...
          <InputField
            ...
            onChange={(event) => update("qtyApproxMt", event.target.value)}
          />

          <SelectField
            label="Product Fit"
            required
            value={form.productFit}
            onChange={(event) => update("productFit", event.target.value)}
            options={[
              {
                label: "Yes",
                value: "true",
          ...
          <SelectField
            ...
            onChange={(event) => update("seriousness", event.target.value)}
            options={[
              {
                label: "Unknown",
                value: "UNKNOWN",
        ...
        <TextAreaField
          ...
          onChange={(event) => update("expectedDemand", event.target.value)}
        />
      ...
      fallback={
        <AppShell>
          <div className="app-card p-6 text-sm text-muted">
            Loading visit report form...
          </div>
        </AppShell>
      }
    >
      <NewVisitReportPageContent />
    </Suspense>
  );
}